...
from dataloader.pt_data_loader.specialdatasets import CityscapesDataset, cityscapes_simple_Dataset
from dataset_util import Kitti
...
class Evaluator:
    def __init__(self, options, model=None):
        ...
        if self.opt.dataset == "cityscapes":
            ...
            self.trainid_to_labels = self._get_trainid_to_label_cityscapes()
            labels_task = self._get_task_labels_cityscapes()
        elif self.opt.dataset == "kitti":
            labels = self._get_labels_kitti()
            self.trainid_to_labels = self._get_trainid_to_label_kitti() 
            labels_task = self._get_task_labels_kitti()
        ...
        if self.opt.dataset == "cityscapes":
            ...
            gt_list=[]
            img_list=[]
            for filename in kitti.val_set_group_by_city['gt']['kitti'].keys():
                gt_list.append(kitti.val_set_group_by_city['gt']['kitti'][filename])
                img_list.append(kitti.val_set_group_by_city['img']['kitti'][filename])
            
            dataset_dict['gt']=gt_list
            dataset_dict['img']=img_list
            test_dataset = cityscapes_simple_Dataset(data_dict=dataset_dict, 
                                                    labels=labels, 
                                                    data_transforms=test_data_transforms,
                                                    dataset = "kitti"
                                                    )
    ...
    def _get_trainid_to_label_cityscapes(self, id=None):
        if id is None:
            id = self.opt.task_to_val

        if id == 1:
            trainid2label = labels_cityscape_seg_train1.gettrainid2label()
        elif id == 2:
            trainid2label = labels_cityscape_seg_train2_only.gettrainid2label()
        elif id == 12:
            trainid2label = labels_cityscape_seg_train2_eval.gettrainid2label()
        elif id == 3:
            trainid2label = labels_cityscape_seg_train3_only.gettrainid2label()
        elif id == 123:
            trainid2label = labels_cityscape_seg_train3_eval.gettrainid2label()

        return trainid2label

    def _get_labels_kitti(self, id=None):
        if id is None:
            id = self.opt.train_set
    ...
    def _get_task_labels_kitti(self, id=None):
        if id is None:
            id = self.opt.task_to_val
        ...
        if id == 1:
            ...
            labels_task = labels_kitti_seg_train2.getlabels()
        elif id == 12:
            labels_task = labels_kitti_seg_train2_eval.getlabels()

        return labels_task

    def _get_trainid_to_label_kitti(self, id=None):
        if id is None:
            id = self.opt.task_to_val
        ...
        if id == 1:
            ...
            trainid2label = labels_kitti_seg_train2_only.gettrainid2label()
        elif id == 12:
            trainid2label = labels_kitti_seg_train2_eval.gettrainid2label()
    ...
    def load_model(self):
        ...
        if self.opt.pseudo_label_mode:
            model_dir = "{}_{}_{}_{}".format(self.opt.dataset, self.opt.multi_exp, self.opt.num_shot, self.opt.num_pseudo_labels)
        else:
            model_dir = "{}_{}_{}".format(self.opt.dataset, self.opt.multi_exp, self.opt.num_shot)

        if not self.opt.multi_exp == 0:
            checkpoint_path = os.path.join(base_path, self.opt.load_model_name, 'models', model_dir, "exp_"+str(self.current_exp),
    ...
    def calculate_metrics(self,epoch=None):
        ...
        average = 0.0
        std = 0.0
        if self.opt.eval_base_model:
            self.init_model()
            self.run_evaluation()
            metrics = self.metric_model.get_scores()
            miou_list.append(metrics['meaniou'])
            results += "base model: \n model_name: {} \n trainnig set: {}\n task_to_val: {}\n".format(self.opt.load_model_name,self.opt.train_set,self.opt.task_to_val)
            results += ("{:<8} | {:<8} | \n").format("miou", "maccuracy")
            results += ("{:<8.3f} | {:<8.3f} |\n").format(metrics['meaniou'], metrics['meanacc'])
            results += ("{:<15} | {:<8} | {:<8} \n").format("category","iou","acc")
            iouDic=metrics['iou']
            accDic=metrics['acc']
            for key in iouDic.keys():
                results += ("{:<15s} | {:<8.3f} | {:<8.3f} \n").format(self.trainid_to_labels[key].name,iouDic[key],accDic[key])
            results += "\n"
        else:
            self.current_exp = 0
            for i in range(self.opt.multi_exp):
                self.metric_model.reset()
                self.current_exp = i+1
                print("-> Running expriment {}".format(self.current_exp))
                self.init_model()
                self.run_evaluation()
        ...
        #     self._save_metrics(epoch, metrics)
                ...
                results += "exp: {} \n model_name: {} \n trainnig set: {}\n task_to_val: {}\n with_pseudo_label: {}\n".format(i+1, self.opt.load_model_name,self.opt.train_set,self.opt.task_to_val,self.opt.pseudo_label_mode)
                results += ("{:<8} | {:<8} | \n").format("miou", "maccuracy")
                results += ("{:<8.3f} | {:<8.3f} |\n").format(metrics['meaniou'], metrics['meanacc'])
                results += ("{:<15} | {:<8} | {:<8} \n").format("category","iou","acc")

                # print(" model_name: {} \n trainnig set: {}\n task_to_val: {}\n".format(self.opt.load_model_name,self.opt.train_set,self.opt.task_to_val))
                # print(("{:<8} | " * 2).format("miou", "maccuracy"))
                # print(("{:<8.3f} | {:<8.3f} |").format(metrics['meaniou'], metrics['meanacc']))
                # print(("{:<15} | {:<8} | {:<8} ").format("category","iou","acc"))
                iouDic=metrics['iou']
                accDic=metrics['acc']
                # print(("{:<8} | {:<8} | {:<8} ").format("category","iou","acc"))
                for key in iouDic.keys():
                    results += ("{:<15s} | {:<8.3f} | {:<8.3f} \n").format(self.trainid_to_labels[key].name,iouDic[key],accDic[key])
                results += "\n"

        average = np.mean(miou_list)
        std = np.std(miou_list)
        results += "Average_miou: {:<8.3f} | STD_miou: {:<8.3f}".format(average, std)
        print(results)
        if self.opt.save_evaluation_results:
            self.save_evaluation_results(results,os.path.join("./Evaluation_results"))
        print("\n-> Done!")
    ...
    def save_evaluation_results(self,results,save_dir):
        self.training_time = time.strftime(
        ...
        if self.opt.eval_base_model:
            ...
            path = os.path.join(save_dir,f"base_model_eval_{self.training_time}.txt")
        else:
            if self.opt.pseudo_label_mode:
                model_dir = "{}_{}_{}_{}".format(self.opt.dataset, self.opt.multi_exp, self.opt.num_shot, self.opt.num_pseudo_labels)
            else:
            ...
            pl_mode = "with_pl" if self.opt.pseudo_label_mode else "without_pl"
            path = os.path.join(save_dir,"{}shot_{}_model{}_to_val_{}_{}_{}.txt".format(self.opt.num_shot,
                                                                                        self.opt.dataset, 
                                                                                        self.opt.train_set, 
                                                                                        self.opt.task_to_val, 
                                                                                        self.training_time, 
                                                                                        pl_mode))
